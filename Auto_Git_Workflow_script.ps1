    ...
    if ($hasLocalChanges) {
        ...
        if ($commitSuccess) {
            if (-not (Test-InternetConnection)) {
                Write-Host "`nAn internet connection is required for remote operation. Exiting..."
                Start-Sleep 3
                exit 1
            }
...
function Test-InternetConnection {
    ...
    try {
        [System.Net.Dns]::GetHostEntry("github.com")
    } catch {
...
function Check-Git {
    ...
    winget install --id=Git.Git --accept-package-agreements --accept-source-agreements --disable-interactivity --scope user
    
    # Check for Git in standard install paths
    ...
    switch ($action) {
        "Init" {
            ...
            if ($LASTEXITCODE -eq 0 -and (Test-Path ".git" -PathType Container)) {
                ...
                @(".env", ".env.production", ".DS_Store", "desktop.ini") | Out-File .gitignore -Encoding utf8 -Append
                git add .gitignore
        ...
        "Clone" {
            # Check internet
            if (-not (Test-InternetConnection)) {
                Show-Message "An internet connection is required to clone." "Git Clone" "Warning"
                return $false
            }
...
function Setup-Remote {
    ...
    if (-not [string]::IsNullOrEmpty($markerPath) -and (Test-Path $markerPath)) {
        Write-Host "`nSkipping remote setup based on marker file: $markerPath"
        return
    }
    ...
    switch ($setupResult) {
        ...
        "Cancel" {
            ...
            try {
                ...
                Write-Host "`nSaved '$markerPath' as a flag to skip future prompts."
...
#
# Function to check for remote changes (simplistic: just checks behind status)
...
function Pull-RemoteChanges {
    ...
    if (-not [string]::IsNullOrEmpty($markerPath) -and (Test-Path $markerPath)) {
        Write-Host "`nSkipping pull changes based on marker file: $markerPath"
        return
    }
...
function Handle-UnpushedCommits {
    ...
    if (-not [string]::IsNullOrEmpty($markerPath) -and (Test-Path $markerPath)) {
        Write-Host "`nSkipping unpushed commits based on marker file: $markerPath"
        return
    }
...
function Commit-Changes {
    ...
    # --- Revised Confirmation Prompt with YesNoCancel ---
...
#
Write-Host "`nStarting AutoGitFlow Script..."
#
Main
#
Write-Host "`nAutoGitFlow Script finished. Exiting..."
Start-Sleep 3
